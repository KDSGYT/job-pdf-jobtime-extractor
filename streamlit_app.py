...
if not uploaded_file:
    st.info("Choose a PDF file to begin. The output can be searched, filtered, and downloaded as CSV.")
    st.stop()

with st.spinner("Reading PDF and extracting PD time, STBY, and split records..."):
    try:
...
filtered = data
if record_type != "All":
    filtered = [row for row in filtered if row["record_type"] == record_type]
if search.strip():
